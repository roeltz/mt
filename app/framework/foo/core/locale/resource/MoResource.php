...
namespace foo\core\locale\resource;
use Exception;
use foo\core\locale\Locale;
use foo\core\locale\Resource;

class MoResource extends Resource {
	...
	function getMessage($message, $localeCode) {
		try {
			if (!isset($this->_data[$localeCode])) {
				$this->loadTranslationData($this->getLocalizedPath($localeCode), $localeCode);
			}
			return @$this->_data[$localeCode][$message];
		} catch(Exception $e) {
			return $message; 
		}
	}
